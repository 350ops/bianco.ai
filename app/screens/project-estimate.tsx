...
import {
  ...
  formatCurrency,
} from '@/lib/itemizedEstimateService';
import {
  PROPERTY_LOCATIONS,
  PROPERTY_AGES,
  PROPERTY_TYPES,
...
export default function ProjectEstimateScreen() {
  ...
  const handleSaveEstimate = async () => {
    ...
    try {
      ...
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      Alert.alert('Estimate Saved!', 'Your project estimate has been saved successfully.', [
  ...
  const isStepValid = () => {
    switch (step) {
      ...
      case 2:
        ...
        return formData.roomType !== '' && formData.width !== '' && formData.length !== '';
      case 3:
  ...
  const getStepDescription = () => {
    switch (step) {
      case 0:
        return 'Tell us about the property location and characteristics.';
      case 1:
        return 'Help us understand the current state and project timeline.';
      case 2:
        return 'Add room details including type and dimensions.';
      case 3:
        return 'Select the finishes and furniture options for this room.';
      default:
        return '';
    }
  };
  ...
  const LiquidGlassCard = ({ children, style }: { children: React.ReactNode; style?: object }) => {
    ...
    return (
      <View style={[styles.cardOuter, style]}>
        <BlurView intensity={40} tint={colors.isDark ? 'dark' : 'light'} style={styles.cardBlur}>
          <LinearGradient
            colors={
              colors.isDark
  ...
  const ProgressBar = () => (
    <View className="mb-6 flex-row gap-2">
      {[0, 1, 2, 3].map((i) => (
        <View key={i} className="h-1 flex-1 overflow-hidden rounded-full bg-border">
          <View
            className={`h-full rounded-full ${i <= step ? 'bg-highlight' : 'bg-transparent'}`}
            style={{
              width: i <= step ? '100%' : '0%',
              backgroundColor: i <= step ? PRIMARY_BLUE : 'transparent',
            }}
          />
        </View>
      ))}
  ...
  const SelectDropdown = ({
    ...
    <View className="mb-4">
      <ThemedText className="mb-2 ml-1 text-sm font-medium">{label}</ThemedText>
      <Pressable
  ...
  const PickerModal = ({
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <Pressable className="flex-1 justify-end bg-black/50" onPress={onClose}>
        <View
          className="max-h-[60%] rounded-t-3xl bg-background"
    ...
    <View className="mb-4">
      ...
      <View className="flex-row items-center">
        <TextInput
          ...
          className="flex-1 rounded-2xl border border-border p-4 text-text"
          style={{
            backgroundColor: colors.isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.03)',
            color: colors.text,
          }}
        />
        {suffix && (
          <ThemedText className="text-light-subtext dark:text-dark-subtext ml-3 font-medium">
            {suffix}
          </ThemedText>
        )}
      </View>
    ...
    <Modal
      ...
      <View className="flex-1 justify-end bg-black/50">
        <View
            ...
            {estimateResult && (
              <>
                ...
                <View
                  ...
                  <ThemedText className="mb-1 text-sm text-white/80">Estimated Total</ThemedText>
                  <ThemedText className="text-4xl font-bold text-white">
                    {formatCurrency(estimateResult.summary.total, estimateResult.currency)}
                  </ThemedText>
                  <ThemedText className="mt-2 text-xs text-white/60">
                    {estimateResult.inputSummary.totalArea.toFixed(1)} m² •{' '}
                    {estimateResult.pricingVersion}
                  </ThemedText>
                </View>

                {/* Category Summary */}
                <View className="mb-6">
                  <ThemedText className="mb-3 font-semibold">Cost Breakdown</ThemedText>
                  <View
                    ...
                    <View className="mb-2 flex-row justify-between">
                      ...
                      <ThemedText className="font-medium">
                        {formatCurrency(estimateResult.summary.materials)}
                      </ThemedText>
                    </View>
                    <View className="mb-2 flex-row justify-between">
                      <ThemedText style={{ color: colors.placeholder }}>Labor</ThemedText>
                      <ThemedText className="font-medium">
                        {formatCurrency(estimateResult.summary.labor)}
                      </ThemedText>
                    </View>
                    <View className="mb-2 flex-row justify-between">
                      ...
                      <ThemedText className="font-medium">
                        {formatCurrency(estimateResult.summary.overhead)}
                      </ThemedText>
                    </View>
                    <View className="mb-2 flex-row justify-between">
                      <ThemedText style={{ color: colors.placeholder }}>Contingency</ThemedText>
                      <ThemedText className="font-medium">
                        {formatCurrency(estimateResult.summary.contingency)}
                      </ThemedText>
                    </View>
                    <View className="flex-row justify-between">
                      <ThemedText style={{ color: colors.placeholder }}>
                        VAT ({(estimateResult.assumptions.taxRate * 100).toFixed(0)}%)
                      </ThemedText>
                      <ThemedText className="font-medium">
                        {formatCurrency(estimateResult.summary.taxTotal)}
                      </ThemedText>
                    </View>
                  </View>
                </View>
                ...
                {/* Project Factors */}
                <View className="mb-6">
                  <ThemedText className="mb-3 font-semibold">Project Factors Applied</ThemedText>
                  <View
                    className="rounded-xl p-4"
                    style={{
                      backgroundColor: colors.isDark
                ...
                {/* Disclaimer */}
                <View className="mb-6 rounded-xl border border-border p-4">
                  <ThemedText
                    className="text-center text-xs leading-relaxed"
                    style={{ color: colors.placeholder }}>
                    This is an itemized estimate based on standard renovation rates. Actual costs
                    may vary based on specific requirements and contractor quotes.
                  </ThemedText>
                </View>
              </>
            )}
          ...
          <View className="flex-row gap-3 border-t border-border px-6 pt-4">
  ...
  return (
    <View className="flex-1 bg-background">
      {/* Header */}
      <View className="px-global pb-4 pt-4" style={{ paddingTop: insets.top + 10 }}>
            ...
            <LiquidGlassCard style={{ marginTop: 12 }}>
              <View style={styles.infoCard}>
                ...
                <ThemedText style={[styles.infoText, { color: colors.placeholder }]}>
                  These factors affect pricing: poor condition requires more prep work, difficult
                  access increases material delivery costs, and urgent timelines may require
                  overtime labor.
                </ThemedText>
              </View>
            </LiquidGlassCard>
      ...
      <View
        ...
        <Button
          title={
            isCalculating ? 'Calculating...' : step === totalSteps - 1 ? 'Get Estimate' : 'Continue'
          }
          variant="primary"
          size="large"
          rounded="xl"
