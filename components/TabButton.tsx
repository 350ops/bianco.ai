      ...
      if (icon) {
        return (
          <View className="relative">
            ...
            {hasBadge && (
              <View className="absolute -right-1.5 -top-1 h-3 w-3 rounded-full bg-red-500" />
            )}
          </View>
        );
      }
        ...
        <View className="relative w-full flex-col items-center justify-center pb-0 pt-4">
          ...
          {labelAnimated ? (
            <Animated.View
              ...
              <Text className="mt-px text-[9px] text-text">{children}</Text>
            </Animated.View>
          ) : (
            <ThemedText className={`mt-px text-[9px] ${labelHidden ? '!text-transparent' : ''}`}>
              {children}
            </ThemedText>
          )}
        </View>
