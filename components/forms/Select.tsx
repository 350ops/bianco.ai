    ...
    return (
      <View className={`relative mb-global ${containerClassName}`} style={style}>
        ...
        <TouchableOpacity
          onPress={handlePress}
          disabled={disabled}
          className={`h-16 flex-row items-center justify-between rounded-lg bg-secondary px-global
                        ${isFocused ? 'border-border' : 'border-border/60'}
                        ${error ? 'border-red-500' : ''}
                        ${disabled ? 'opacity-50' : ''}
                        ${className}`}>
          <ThemedText className={selectedOption ? 'text-base text-text' : 'text-base text-text'}>
