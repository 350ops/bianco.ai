...
export function DateTimeSection() {
  ...
  return (
    <Section title="📅 Date & Time Management">
      <Text size={16}>{`Current Selection: ${selectedDate.toLocaleString()}`}</Text>

      <Picker
        label="Display Style"
        selection={displayOptions[displayStyle]}
        onSelectionChange={(selection) => {
          setDisplayStyle(displayOptions.indexOf(selection as 'compact' | 'graphical' | 'wheel'));
        }}
      />
      ...
      <Picker
        ...
        onSelectionChange={(selection) => {
          setPickerType(typeOptions.indexOf(selection as 'date' | 'hourAndMinute' | 'dateAndTime'));
        }}
      />
